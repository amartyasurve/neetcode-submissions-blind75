...
public:
    void bfs(int x,int y,int n,int m,vector<vector<int>>&visited,vector<vector<char>>&grid){
     ...
     while(!q.empty()){
        ...
        q.pop();
        for(int i=0;i<4;i++){
            if(xc+xarray[i]>=0 && xc+xarray[i]<n && yc+yarray[i]>=0 && yc+yarray[i]<m && visited[xc+xarray[i]][yc+yarray[i]]==0){
              if(grid[xc+xarray[i]][yc+yarray[i]]=='1'){
                q.push({xc+xarray[i],yc+yarray[i]});
                visited[xc+xarray[i]][yc+yarray[i]]=1;
                }
            }
        }
     }
